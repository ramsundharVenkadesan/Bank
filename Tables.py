...
from sqlalchemy import Column, String, Boolean, Float, Integer, ForeignKey, Enum # Import the Column class to define columns and its data types
...
class User(Base): # Parent-User table that inherits from the Base class used to construct the database itself
    ...
    username = Column(String, unique=True, index=True, primary_key=True) # Column to store user's username that must be unique because its the primary key of the table to uniquely identify each record
...
class Transaction(Base): # Child-Transaction table that inherits from the Base class used to construct the database itself
    ...
    owner_id = Column(String, ForeignKey("User.username")) # Column that references primary key of User's database table (Foreign Key) to link two tables together